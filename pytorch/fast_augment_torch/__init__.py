...
class CenterCrop:
    ...
    def __call__(self, x):
        x, _ = self.backend(x, _empty_tensor, output_size=self.size, is_float32_output=False)
        return x
...
class FastAugment:
    ...
    def __init__(self,
            ...
            flip_vertically=False,
            seed=0):
        """ Creates a FastAugment object used to apply a set of random geometry and color transformations to batches of images.

            Args:
                translation:        Normalized image translation range along X and Y axis. `0.1` corresponds to a random shift by at most 10% of the image size in both directions (default).
                                    If one value given, the same range applies for X and Y axes.
                scale:              Scaling factor range along X and Y axes. `0.1` corresponds to stretching the images by a random factor of at most 10% (default).
                                    If one value given, the applied scaling keeps the aspect ratio: the same factor is used along X and Y axes.
                ...
                perspective:        Perspective distortion range setting the maximum tilting and panning angles in degrees.
                                    ...
                                    If one number is given, the same range applies for both axes. The default value is 15 degrees.
                ...
                hue:                Hue shift range in degrees. The image pixels color hues are shifted by a random angle smaller than `hue`.
                                    A hue shift of +/-120 degrees transforms green in red/blue and vice versa. The default value is 10 degrees.
                ...
                cutout_size:        A list specifying the normalized size range CutOut area width and height are sampled from.
                                    `[0.3, 0.5]` range produces a rectangle of 30% to 50% of image size on every side (default). If empty list is passed, CutOut application is disabled.
                mixup:              Probability of mixup being applied to a given input image. Mixup is disabled by default (`mixup` is set to zero).
                                    ...
                                    Requires the input labels `y`. If not provided, an exception is thrown.
    ...
    def __call__(self, x, y=None, output_size=None, output_type=torch.float32):
        """ Applies a sequence of random transformations to images in a batch.
            ...
            Args:
                ...
                output_type:        Output image datatype. Can be `float32` or `uint8`. Default: `float32`.
        """
        if output_type not in [torch.uint8, torch.float32]:
            raise ValueError(f"Unsupported output type: {output_type}")

        x_, y_ = self.backend(input=x,

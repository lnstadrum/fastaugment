...
namespace fastaugment {
     ...
     * Define the random distribution to sample per-image augmentation parameters from.
     */
    typedef struct {
    ...
    private:
        ...
        static inline val_t roundUp(val_t value, mod_t modulo) {
    ...
    protected:
         ...
         * Samples augmentation parameters per image in the batch, sends them to GPU memory, launches the CUDA kernel.
         ...
         * @param batchSize         Batch size; 0 if 3-dimensional input tensor is given
        ...
        void run(
            ...
            std::uniform_real_distribution<float>
                xShiftFactor(-settings.translation[0], settings.translation[0]),
                ...
                panAngle(-settings.perspective[1], settings.perspective[1]),
                cutoutApplication(0.0f, 1.0f),
                cutoutPos(0.0f, 1.0f),
                cutoutSize(settings.cutout[0], settings.cutout[1]),
                mixupApplication(0.0f, 1.0f),
                mixShift(-0.1f, 0.1f),
                hueShift(-settings.hue, settings.hue),
            ...
            for (size_t i = 0; i < paramsCpu.size(); ++i) {
                ...
                const float
                    ...
                    scaleY = settings.isotropicScaling ? scaleX : yScaleFactor(rnd);
            ...
            try {
                compute(stream,
                        bufferPtr, outputPtr,                   // input and output pointers

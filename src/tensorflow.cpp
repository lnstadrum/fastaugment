...
#define CUDASSERT(CALL, MSG) OP_REQUIRES(context, CALL == cudaSuccess, errors::Internal(MSG));

template <typename val_t, typename mod_t> inline val_t roundUp(val_t value, mod_t modulo)
{
...
class FastAugmentTFOpKernel : public OpKernel,
                              ...
                              private fastaugment::Settings
{
        ...
        if (context->GetAttr(attribute, &listArg) != Status::OK())
        {
            context->CtxFailure(
                errors::InvalidArgument("Cannot get '" + std::string(attribute) +
                                        "' attribute value. Expected a list of floating point values."));
            return 0;
        }

        if (listArg.size() > 2)
            context->CtxFailure(errors::InvalidArgument("Cannot interpret '" + std::string(attribute) +
                                                        "' attribute value of " + std::to_string(listArg.size()) +
                                                        " elements"));
        a = b = 0;
        if (listArg.size() == 1)
            a = b = listArg[0];
  ...
  public:
    explicit FastAugmentTFOpKernel(OpKernelConstruction *context) : OpKernel(context)
    {
        // get output size
        OP_REQUIRES_OK(context, context->GetAttr("output_size", &outputSize));
        if (!outputSize.empty() && outputSize.size() != 2)
            context->CtxFailure(errors::InvalidArgument("Invalid output_size: expected an empty list "
                                                        "or a list of 2 entries, got " +
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        OP_REQUIRES(context, inputShape.dims() == 3 || inputShape.dims() == 4,
                    errors::InvalidArgument("Expected a 3- or 4-dimensional input tensor, got " +
                                            std::to_string(inputShape.dims()) + " dimensions"));

        // get input sizes
        const bool isBatch = inputShape.dims() == 4;
        const int64_t batchSize = isBatch ? inputShape.dim_size(0) : 0,
        ...
        OP_REQUIRES(context, inputChannels == 3,
                    errors::InvalidArgument("Expected a 3-channel input tensor, got " + std::to_string(inputChannels) +
                                            " channels"));
        ...
        if (!noLabels)
        {
            OP_REQUIRES(context, labelsShape.dims() == 2,
                        errors::InvalidArgument("Expected a 2-dimensional input_labels tensor, got " +
                                                std::to_string(labelsShape.dims()) + " dimensions"));
            OP_REQUIRES(context, labelsShape.dim_size(0) == batchSize,
                        errors::InvalidArgument("First dimension of the input labels tensor is expected "
                                                "to match the batch size, but got " +
                                                std::to_string(labelsShape.dim_size(0))));
        }
        ...
        if (isBatch)
            OP_REQUIRES_OK(context,
                           context->allocate_output(0, {batchSize, outputHeight, outputWidth, 3}, &outputTensor));
        else
...
#define REGISTER_KERNEL(IN_T, OUT_T)                                                                                   \
    REGISTER_KERNEL_BUILDER(Name("Augment")                                                                            \
                                .Device(DEVICE_GPU)                                                                    \
...
REGISTER_OP("Augment")
    ...
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext *ctx) {
        ...
        if (inputRank != 3 && inputRank != 4)
            errors::InvalidArgument("Expected a 3- or 4-dimensional input tensor, got " + std::to_string(inputRank) +
                                    " dimensions");
        ...
        if (!outputSize.empty() && outputSize.size() != 2)
            return errors::InvalidArgument("Invalid output_size: expected an empty list or a list of 2 "
                                           "entries, got " +
                                           std::to_string(outputSize.size()));
        ...
        if (outputSize.empty())
            ...
            ctx->set_output(0, ctx->MakeShape({outputSize[0], outputSize[1], 3}));
        else
            ctx->set_output(0, ctx->MakeShape({ctx->Dim(ctx->input(0), 0), outputSize[0], outputSize[1], 3}));

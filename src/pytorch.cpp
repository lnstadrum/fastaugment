...
void normalizePair(const std::vector<float> input, const char* attrName, float& x, float& y) {
    if (input.size() == 0 || input.size() > 2)
        throw std::invalid_argument("Expecting one or two entries in '" + std::string(attrName) + "' argument, but got " + std::to_string(input.size()) + ".");
...
public:
    TorchTempGPUBuffer(size_t sizeBytes, c10::cuda::CUDAStream stream):
        ptr(c10::cuda::CUDACachingAllocator::get()->allocate(sizeBytes)),
        stream(stream)
    { }
...
public:
    TorchKernel(
        ...
        float prescale,
        float rotation,
        const std::vector<float>& perspective,
        float cutout,
        const std::vector<float>& cutoutSize,
        float mixup,
        float mixupAlpha,
        float hue,
        float saturation,
    ...
    std::vector<torch::Tensor> operator()(
        ...
        bool isFloat32Output
    ) {
        // check output size
        if (!outputSize.empty() && outputSize.size() != 2)
            throw std::invalid_argument("Invalid output_size: expected an empty list or a list of 2 entries, got " + std::to_string(outputSize.size()));

        // check the input tensor
        if (input.dim() != 3 && input.dim() != 4)
            throw std::invalid_argument("Expected a 3- or 4-dimensional input tensor, got " + std::to_string(input.dim()) + " dimensions");
        if (!input.is_cuda())
        ...
        const int64_t
            batchSize = isBatch ? input.size(0) : 0,
            inputHeight = input.size(isBatch ? 1 : 0),
            inputWidth = input.size(isBatch ? 2 : 1),
            inputChannels = input.size(isBatch ? 3 : 2),
        ...
        if (inputChannels != 3)
            throw std::invalid_argument("Expected a 3-channel channels-last (NHWC) input tensor, got " + std::to_string(inputChannels) + " channels");
        ...
        if (!noLabels) {
            if (labels.dim() != 2)
                throw std::invalid_argument("Expected a 2-dimensional input_labels tensor, got " + std::to_string(labels.dim()) + " dimensions");
            if (labels.size(0) != batchSize)
                throw std::invalid_argument("First dimension of the input labels tensor is expected to match the batch size, but got " + std::to_string(labels.size(0)));
            if (!labels.is_cpu())
                throw std::invalid_argument("Expected an input_labels tensor stored in RAM (likely missing a .cpu() call)");
        ...
        auto outputOptions = torch::TensorOptions().device(input.device()).dtype(isFloat32Output ? torch::kFloat32 : torch::kUInt8);
        ...
        launchKernel(
            ...
            output,
            inputLabelsPtr,
            outputLabels.data_ptr<float>(),
            batchSize,
            inputHeight,
            inputWidth,
            outputHeight,
            outputWidth,
            noLabels ? 0 : labels.size(1),
            stream.stream(),
            stream
        );
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module){
    py::class_<TorchKernel>(module, "FastAugment")
        .def(py::init<const std::vector<float>&,
                      ...
                      bool,
                      int>(),
            py::arg("translation") = std::vector<float>{0},
            py::arg("scale") = std::vector<float>{0},
            py::arg("prescale") = 1.0f,
            py::arg("rotation") = 0.0f,
            py::arg("perspective") = std::vector<float>{0},
            py::arg("cutout") = 0.0f,
            py::arg("cutout_size")  = std::vector<float>{},
            py::arg("mixup") = 0.0f,
            py::arg("mixup_alpha") = 0.4f,
            py::arg("hue") = 0.0f,
            py::arg("saturation") = 0.0f,
            py::arg("brightness") = 0.0f,
            py::arg("gamma_corr") = 0.0f,
            py::arg("color_inversion") = false,
            py::arg("flip_horizontally") = false,
            py::arg("flip_vertically") = false,
            py::arg("seed") = 0
        )

        .def("__call__", &TorchKernel::operator(),
            py::arg("input"),
            py::arg("input_labels"),
            py::arg("output_size"),
